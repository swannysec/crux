...
print_tag_cleanup_reminder() {
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"crux DEV ${tag}.app/Contents/MacOS/crux DEV\""
  ...
  echo "  pkill -f \"crux DEV ${current_slug}.app/Contents/MacOS/crux DEV\""
