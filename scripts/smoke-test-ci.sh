...
# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/crux DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/crux DEV"
if [ ! -x "$BINARY" ]; then
...
rm -f "$SOCKET_PATH"
pkill -x "crux DEV" 2>/dev/null || true
sleep 1
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  pgrep -la "crux" || echo "No crux processes found"
  exit 1
fi
